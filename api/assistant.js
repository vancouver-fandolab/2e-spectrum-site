...
export default async function handler(req, res) {
  ...
  try {
    ...
    // NOTE ON MODEL NAME: Google periodically retires older Gemini model
    // versions (this happened to gemini-2.5-flash around June 2026, which is
    // why this file was updated to gemini-3.5-flash). If the assistant ever
    // reverts to canned/offline-sounding answers again, check whether this
    // model string has since been deprecated at
    // https://ai.google.dev/gemini-api/docs/models — a deprecated model
    // returns an error here, which silently triggers the frontend's local
    // keyword-matching fallback (by design, so the widget never breaks
    // visibly — but it does mean failures here are easy to miss).
